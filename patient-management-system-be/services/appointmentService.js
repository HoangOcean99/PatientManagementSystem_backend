...
export const updateOrReschedule = async (appointment_id, updates, currentUser) => {
  //Kiểm tra tồn tại 
  const { data: appointment, error: fetchError } = await supabase
    ...
    .single()
  if (fetchError || !appointment_id) throw new AppError('Lịch khám không tồn tại', 500);

  //Kiểm tra quyền hạng
  const canGlobalEdit = ['ADMIN', 'RECEPTIONIST'].includes(currentUser.role)
  const isDoctorAssigned = currentUser.role === 'DOCTOR' && currentUser.user_id === appointment.doctor_id;
  const isOwned = currentUser.user_id === appointment.patient_id
  if (!canGlobalEdit && !isOwned && !isDoctorAssigned) throw new AppError('Bạn không có quyền', 500)


  //Thực hiện cập nhật 
  const { data: recheduledAppointment, error: updateError } = await supabase
    .from('Appointments')
    .update({
      ...updates,
  ...
  if (updateError) throw new AppError('Cập nhật thất bại', 400)
  return recheduledAppointment;
}
